...
import type { Goal } from "@/types/entry";
...
export async function createGoal(goal: Goal) {
  if (hasSupabase()) { const {error}=await supabaseAdmin().from("goals").insert({id:goal.id,title:goal.title,metric:goal.metric,target:goal.target,current:goal.current,period:goal.period,category:goal.category||null,deadline:goal.deadline||null,is_public:goal.isPublic,created_at:goal.createdAt}); if(error) throw error; }
  else { await fs.mkdir(path.dirname(file),{recursive:true}); await fs.writeFile(file,JSON.stringify([goal,...await getGoals(true)],null,2)+"\n"); }
}
