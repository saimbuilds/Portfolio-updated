...
export function Studio({ initialEntries }: { initialEntries: Entry[] }) {
  ...
  return (
    <div className="studio-shell shell">
      ...
      <section className="capture"><div className="mode-selector"><button type="button" className={`mode-card${!manual ? " mode-active" : ""}`} onClick={() => setManual(false)}><span className="mode-icon">⏱</span><strong>Start Timer</strong><p>Begin a live focus block. Stop it when you&apos;re done.</p></button><button type="button" className={`mode-card${manual ? " mode-active" : ""}`} onClick={() => setManual(true)}><span className="mode-icon">⚡</span><strong>Add Instantly</strong><p>Already worked? Log it right now without a timer.</p></button></div><form onSubmit={submit}>
